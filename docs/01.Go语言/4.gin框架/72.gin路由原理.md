---
title: gin路由原理
date: 2022-06-25 22:28:33
permalink: /pages/7e7998/
categories:
  - Go语言
  - gin框架
tags:
...
- radix树就是针对这种稀疏的长整型数据查找，能快速且节省空间地完成映射。借助于Radix
  树，我们可以实现对于长整型数据类型的路由。
- 利用radix树可以根据一个长整型（比如一个长ID)快速查找到其对应的对象指针。这比用
  hash映射来的简单，也更节省空间，使用hash映射hash函数难以设计，不恰当的hash函数可
  能增大冲突，或浪费空间。
- radix tree是一种多叉搜索树，树的叶子结点是实际的数据条目。每个结点有一个固定的指针
  指向子结点（每个指针称为槽slot，n为划分的基的大小）
...
![image-20220426171450709](https://raw.githubusercontent.com/daniuEvan/pictrues/main/Typora/image-20220426171450709.png)
...
![addroute](https://raw.githubusercontent.com/daniuEvan/pictrues/main/Typora/addroute.gif)
